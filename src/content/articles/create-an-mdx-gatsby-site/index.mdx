...
Start by creating a new GitHub repository and cloning it locally (Optional).
...
Restart your server (`CTRL+C` then `npm start`) and navigate to [http://localhost:8000/page-2](http://localhost:8000/page-2) to observe the MDX page working.
...
Restart your server and navigate to [localhost:8000/content-page](http://localhost:8000/content-page) to see it working.
...
We're now creating pages from React components and MDX files within the content directory (and sub-directories)! 🎉 However, we don't have a way to interact with those pages from our components. Adding this feature is useful for creating things like a list of recent blog posts.
...
Restart your development server, and observe the output of the console log. We then use this data to render a list of content pages.
